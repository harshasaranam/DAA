...
public class ProjectMain {
	static Map<String,LinkWeightValues> map=new Hashtable<String,LinkWeightValues>();
	public static void main(String[] args) throws Exception {
	    BufferedReader br = new BufferedReader(new FileReader("C:/Users/nicen/workspace/daa/src/input.txt"));
	    String line = null;
	    ...
	    while ((line = br.readLine()) != null) {
	      ...
	      if(firstline){
    		  ...
    		  map.put(key1, new LinkWeightValues(vertex1,vertex2,mean,variance,csquare));
    		  String key2=vertex2+""+vertex1;
    		  map.put(key2, new LinkWeightValues(vertex2,vertex1,mean,variance,csquare));
	    ...
	    CallDikjstra(totalVertices,sourceName,destinationName,Criteria.MEAN_VALUE);
	    CallDikjstra(totalVertices,sourceName,destinationName,Criteria.OPTIMIST);
	    CallDikjstra(totalVertices,sourceName,destinationName,Criteria.PESSIMIST);
	    CallDikjstra(totalVertices,sourceName,destinationName,Criteria.DOUBLY_PESS);
	    CallDikjstra(totalVertices,sourceName,destinationName,Criteria.STABLE);
	    //call another method with own criteria
	    
	    
	  }
	  public static void CallDikjstra(int totalVertices,int sourceName,int destinationName,Criteria criteria){
		  DikjstraMinHeap.Graph graph=new DikjstraMinHeap.Graph(totalVertices+1); 
		    for (int i = 1; i < (totalVertices+1); i++) {
	            graph.addVertex(i);
	        }
		    System.out.println(criteria);
		    for(Map.Entry<String, LinkWeightValues> entry:map.entrySet()){    
		    	LinkWeightValues d=entry.getValue();
		        double weight=0;
		    ...
		    for(int i=0;i<edges.length;i++){
		    	...
		    	LinkWeightValues d=map.get(Keys[0]+Keys[1]);
